...
見た目の手ボーンに対し、実行時スクリプトでコライダを自動生成・配置し、物理手プレハブに依存せず鍵盤との当たり判定準備を整える。
...
1. **コライダ自動生成**
   - 実行時スクリプト（例: FingerColliderBuilder）で各指（親指〜小指）に 1〜3 本のカプセル／コライダを TIP から根元方向へ生成・配置できる。
...
3. **Rigidbody設定**
   - 手のルート（または適切な親）に kinematic Rigidbody が付与され、生成されたコライダが安定して移動する。

4. **パフォーマンス**
   - 物理手プレハブ（HandPhysics/HandCollider）は使用せず、自動生成コライダで 60fps 目標を維持できる。

5. **デバッグ確認**
   - シーン内で生成されたコライダの有効/無効やレイヤー確認ができ、少なくとも一度は鍵盤オブジェクトと当たり判定が取れることを確認する。
