...
- ピアノ用シーン `hapticpiano.unity` 内での手の振る舞い
...
3.  `hapticpiano.unity` を主要シーンとする 
4. SteamVR の制約に反しない範囲で助言する
...
- rm -rf 等の破壊的 CLI の提案
- セキュリティリスクのあるコード生成
- Scene(.unityファイルの変更)シーンを変更はユーザーにしてもらう
- .metaファイルの追加
- .unity を絶対に git checkout で戻さない
